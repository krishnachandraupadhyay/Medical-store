...
@section('title', 'Lost & Missing Stock Control')
...
<div class="h-full overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-bold text-amber-600 uppercase tracking-wider bg-amber-50 px-2.5 py-1 rounded-lg border border-amber-100">
                    Loss / Shrinkage Control
                </span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-black text-[#1e2746] tracking-tight mt-1">Lost & Missing Stock Control</h1>
            <p class="text-xs sm:text-sm text-[#64748b] mt-0.5">
                Record and audit unexplained inventory shrinkage, theft, or unaccounted missing medicine stock with mandatory audit logs.
            </p>
        </div>

        <div class="flex items-center gap-3">
            <div class="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-2 text-right">
                <div class="text-[10px] font-extrabold uppercase tracking-wider text-amber-600">Total Lost Units</div>
                <div class="text-xl font-black text-amber-700 font-mono">{{ number_format($totalLostUnits) }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
        <h2 class="text-base font-black text-[#1e2746] tracking-tight mb-4">Record Lost / Missing Stock Write-Off</h2>
        <form method="POST" action="{{ route('store.inventory.lost.store') }}" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-4">
                ...
                <div class="sm:col-span-2">
                    <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-500 mb-1.5">Missing Qty *</label>
                ...
                <div class="sm:col-span-4">
                    <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-500 mb-1.5">Shrinkage / Loss Reason *</label>
                    <input type="text" name="reason" required placeholder="e.g. Unexplained shelf shortage, theft suspected"
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-end">
                <div class="flex-1 w-full">
                    <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-500 mb-1.5">Detailed Investigation Notes</label>
                    <input type="text" name="notes" placeholder="Optional notes on stockroom audit or investigation..."
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs sm:text-sm focus:outline-none focus:border-[#4b55c8] bg-slate-50/50">
                </div>
                <button type="submit" class="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs shadow-md shadow-amber-600/20 transition whitespace-nowrap">
                    Write Off Lost Stock
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100">
            <h3 class="text-base font-black text-[#1e2746]">Lost Stock Write-Off History</h3>
        </div>
            ...
            <table class="w-full text-left text-xs sm:text-sm">
                <thead class="bg-slate-50 text-[11px] font-extrabold uppercase tracking-wider text-slate-400 border-b border-slate-100">
                    <tr>
                        ...
                        <th class="py-3.5 px-6 text-right">Lost Qty</th>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-12 text-center text-slate-400">
                                No lost or missing stock write-offs recorded.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
