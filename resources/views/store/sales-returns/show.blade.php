...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-1">Returned on {{ $salesReturn->return_date->format('d M Y') }} against Sale Invoice #{{ $salesReturn->sale?->invoice_number }}</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('store.sales.show', $salesReturn->sale_id) }}" class="px-3.5 py-2 text-xs font-bold text-[#4b55c8] bg-[#eef2fd] rounded-xl hover:bg-[#e0e8fc] transition">
                View Original Sale
            </a>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm">
            ...
            <p class="text-xl font-black text-slate-900 mt-1">₹{{ number_format($salesReturn->grand_total, 2) }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm">
            <p class="text-xs font-bold uppercase text-slate-400">Refund Amount</p>
            <p class="text-xl font-black text-rose-600 mt-1">₹{{ number_format($salesReturn->refund_amount, 2) }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm">
            <p class="text-xs font-bold uppercase text-slate-400">Customer</p>
            <p class="text-base font-bold text-slate-900 mt-1">{{ $salesReturn->customer?->name ?: 'Walk-in Customer' }}</p>
        </div>
    </div>
            ...
            <table class="w-full text-left text-xs sm:text-sm">
                <thead class="bg-slate-50 text-slate-500 font-bold border-b border-slate-100">
                    <tr>
                        ...
                        <th class="py-3 px-4">Restocked Batch</th>
                ...
                <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                    ...
                    <tr>
                        ...
                        <td class="py-3 px-4 font-black text-emerald-600">{{ $item->quantity }} units</td>
