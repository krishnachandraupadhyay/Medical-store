...
<div class="h-full overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-600">Edit Role</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-black text-[#1e2746] tracking-tight">Edit Custom Role</h1>
            <p class="text-xs sm:text-sm text-[#64748b] mt-0.5">
                Modify role details and configure permitted operations across store modules.
            </p>
        </div>
    ...
    <form method="POST" action="{{ route('store.roles.update', $role) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" required class="w-full px-4 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4b55c8]/20 focus:border-[#4b55c8] transition"/>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="description" name="description" value="{{ old('description', $role->description) }}" class="w-full px-4 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4b55c8]/20 focus:border-[#4b55c8] transition"/>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8 space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-100 pb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Checked permissions grant operational authorization to users with this role.</p>
                </div>
                            ...
                            @foreach ($permissions as $permission)
                                @php
                                    $isChecked = is_array(old('permissions'))
                                        ? in_array($permission->slug, old('permissions'))
                                        : in_array($permission->slug, $assignedPermissionSlugs);
                                @endphp
                                <label class="flex items-start gap-2.5 p-2 rounded-xl hover:bg-white transition cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->slug }}" {{ $isChecked ? 'checked' : '' }} class="mt-0.5 rounded border-slate-300 text-[#4b55c8] focus:ring-[#4b55c8] group-checkbox-{{ Str::slug($group) }}">
                                    <div>
