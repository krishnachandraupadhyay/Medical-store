...
@section('title', 'Edit Draft Purchase - ' . $purchase->invoice_number)
...
<div class="h-full overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2">
                <a href="{{ route('store.purchases.show', $purchase) }}" class="text-xs font-bold text-[#4b55c8] hover:underline flex items-center gap-1">
                    ← Back to Purchase Invoice
                </a>
            </div>
            <h1 class="text-2xl sm:text-3xl font-black text-[#1e2746] tracking-tight mt-1">Edit Draft Purchase</h1>
            <p class="text-xs sm:text-sm text-[#64748b] mt-0.5">Modify invoice details, items, quantities, or prices before finalizing.</p>
        </div>
    </div>
    ...
    <form id="purchaseForm" method="POST" action="{{ route('store.purchases.update', $purchase) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="invoice_number" value="{{ old('invoice_number', $purchase->invoice_number) }}" required class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs sm:text-sm font-mono focus:border-[#4b55c8] outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5">Purchase / Invoice Date <span class="text-rose-500">*</span></label>
                    <input type="date" name="purchase_date" value="{{ old('purchase_date', $purchase->purchase_date->toDateString()) }}" required class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs sm:text-sm focus:border-[#4b55c8] outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5">Overall Bill Discount (₹)</label>
                    <input type="number" step="0.01" min="0" name="discount" id="overallDiscount" value="{{ old('discount', $purchase->discount) }}" class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs sm:text-sm font-mono focus:border-[#4b55c8] outline-none">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5">Notes / Terms</label>
                <input type="text" name="notes" value="{{ old('notes', $purchase->notes) }}" class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs sm:text-sm focus:border-[#4b55c8] outline-none">
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-0.5">Modify batches and items for this draft purchase.</p>
                </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('store.purchases.show', $purchase) }}" class="px-5 py-2.5 rounded-2xl border border-slate-200 text-slate-600 hover:bg-slate-50 font-bold text-xs sm:text-sm transition">
                Cancel
            </a>
...
<script>
    ...
    function addRow(data = {}) {
        ...
        tr.innerHTML = `
            ...
            <td class="py-2 px-3">
                <input type="date" name="items[${itemIndex}][expiry_date]" value="${data.expiry_date ? data.expiry_date.substring(0, 10) : ''}" required class="w-full px-2 py-1.5 rounded-xl border border-slate-200 text-xs focus:border-[#4b55c8] outline-none">
            </td>
            <td class="py-2 px-3">
