...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Sales Return Report</h1>
            <p class="text-sm text-slate-500 mt-1">Audit of customer medicine returns and inventory restocks.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm">
        <form method="GET" action="{{ route('store.reports.sales-returns') }}" class="grid grid-cols-1 sm:grid-cols-5 gap-3">
            <div>
            ...
            <div>
                ...
                <select name="customer_id" class="w-full text-xs rounded-xl border border-slate-200 p-2 outline-none">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('store.reports.sales-returns') }}" class="px-3 py-2 rounded-xl border border-slate-200 text-slate-600 text-xs font-bold hover:bg-slate-50 transition">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full text-left text-xs sm:text-sm">
                <thead class="bg-slate-50 text-slate-500 font-bold border-b border-slate-100">
                    <tr>
                        ...
                        <th class="py-3 px-3.5">Original Sale</th>
                ...
                <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50/70 transition">
                        <td class="py-2.5 px-3.5 font-mono font-bold text-[#4b55c8]">
                            <a href="{{ route('store.sales-returns.show', $r->id) }}" class="hover:underline">
                                {{ $r->return_number }}
                            </a>
                        </td>
                        <td class="py-2.5 px-3.5 font-mono font-bold text-slate-800">{{ $r->sale?->invoice_number }}</td>
                        <td class="py-2.5 px-3.5 font-semibold text-slate-900">{{ $r->customer?->name ?: 'Walk-in' }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="py-12 text-center text-slate-400">No sales return records found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
