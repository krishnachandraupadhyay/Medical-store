...
@section('title', 'Edit Notification - ' . $notification->title)
...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                ...
                <span>/</span>
                <a href="{{ route('super-admin.notifications.show', $notification) }}" class="hover:text-[#4b55c8] transition truncate max-w-xs">{{ $notification->title }}</a>
                <span>/</span>
                <span class="text-slate-600 font-semibold">Edit</span>
            </div>
            <h2 class="text-xl sm:text-2xl font-bold text-[#1e2746] tracking-tight">Edit Notification</h2>
            <p class="text-xs sm:text-sm text-[#64748b] mt-1">Modify draft or scheduled platform notification #{{ $notification->id }}.</p>
        </div>

        <div class="flex items-center gap-2">
            <a href="{{ route('super-admin.notifications.show', $notification) }}" class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition">
                View Details
            </a>
    ...
    <div class="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm">
        <form action="{{ route('super-admin.notifications.update', $notification) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="type" name="type" required class="w-full px-4 py-2.5 text-xs sm:text-sm rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#4b55c8] transition @error('type') border-rose-400 @enderror">
                            @foreach ($types as $t)
                                <option value="{{ $t->value }}" {{ old('type', $notification->type->value) === $t->value ? 'selected' : '' }}>
                                    {{ $t->icon() }} {{ $t->label() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="priority" name="priority" required class="w-full px-4 py-2.5 text-xs sm:text-sm rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#4b55c8] transition @error('priority') border-rose-400 @enderror">
                            @foreach ($priorities as $p)
                                <option value="{{ $p->value }}" {{ old('priority', $notification->priority->value) === $p->value ? 'selected' : '' }}>
                                    {{ $p->label() }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="message"
                              ...
                              rows="5"
                              required
                              class="w-full px-4 py-2.5 text-xs sm:text-sm rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#4b55c8] transition @error('message') border-rose-400 bg-rose-50/30 @enderror">{{ old('message', $notification->message) }}</textarea>
                    @error('message')
                        <p class="text-[11px] text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="target_type" name="target_type" required class="w-full px-4 py-2.5 text-xs sm:text-sm rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#4b55c8] transition">
                            <option value="all_stores" {{ old('target_type', $notification->target_type->value) === 'all_stores' ? 'selected' : '' }}>
                                All Stores (Platform-wide broadcast)
                            </option>
                            <option value="specific_store" {{ old('target_type', $notification->target_type->value) === 'specific_store' ? 'selected' : '' }}>
                                Specific Store
                            </option>
                        </select>
                    </div>
                    ...
                    <div id="storeSelectorContainer" class="space-y-1.5 {{ old('target_type', $notification->target_type->value) === 'specific_store' ? '' : 'hidden' }}">
                        ...
                        <select id="store_id" name="store_id" class="w-full px-4 py-2.5 text-xs sm:text-sm rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#4b55c8] transition @error('store_id') border-rose-400 @enderror">
                            ...
                            @foreach ($stores as $store)
                                <option value="{{ $store->id }}" {{ (string)old('store_id', $notification->store_id) === (string)$store->id ? 'selected' : '' }}>
            ...
            <div class="space-y-4 pt-2">
                ...
                @php
                    $currentMode = match ($notification->status) {
                        \App\Enums\NotificationStatus::SCHEDULED => 'schedule',
                        \App\Enums\NotificationStatus::SENT => 'now',
                        default => 'draft',
                    };
                @endphp
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="delivery_mode" name="delivery_mode" required class="w-full px-4 py-2.5 text-xs sm:text-sm rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#4b55c8] transition">
                            <option value="now" {{ old('delivery_mode', $currentMode) === 'now' ? 'selected' : '' }}>
                                🚀 Send Immediately (Publish Now)
                            </option>
                            <option value="schedule" {{ old('delivery_mode', $currentMode) === 'schedule' ? 'selected' : '' }}>
                                ⏰ Schedule for Later
                            </option>
                            <option value="draft" {{ old('delivery_mode', $currentMode) === 'draft' ? 'selected' : '' }}>
                                📝 Save as Draft
                            </option>
                        </select>
                    </div>
                    ...
                    <div id="scheduledTimeContainer" class="space-y-1.5 {{ old('delivery_mode', $currentMode) === 'schedule' ? '' : 'hidden' }}">
                        ...
                        <input type="datetime-local"
                               id="scheduled_at"
                               name="scheduled_at"
                               value="{{ old('scheduled_at', $notification->scheduled_at?->format('Y-m-d\TH:i') ?? now()->addHours(1)->format('Y-m-d\TH:i')) }}"
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                <a href="{{ route('super-admin.notifications.show', $notification) }}" class="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition">
                    Cancel
                </a>
