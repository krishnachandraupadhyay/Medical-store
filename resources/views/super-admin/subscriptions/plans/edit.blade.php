...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('super-admin.subscriptions.plans.show', $plan) }}" class="inline-flex items-center gap-2 text-xs font-semibold text-[#64748b] hover:text-[#1e2746] transition">
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50/50">
            <h2 class="text-lg sm:text-xl font-bold text-[#1e2746] tracking-tight">Edit Subscription Tier: {{ $plan->name }}</h2>
            <p class="text-xs text-[#64748b] mt-0.5">Modify pricing parameters, resource thresholds, and active platform modules.</p>
        </div>

        <form action="{{ route('super-admin.subscriptions.plans.update', $plan) }}" method="POST" class="p-6 sm:p-8 space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        ...
                        >
                            @foreach ($billingCycles as $cycle)
                                <option value="{{ $cycle->value }}" {{ old('billing_cycle', $plan->billing_cycle->value) === $cycle->value ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        >
                            @foreach ($planStatuses as $status)
                                <option value="{{ $status->value }}" {{ old('status', $plan->status->value) === $status->value ? 'selected' : '' }}>
                        ...
                        <div class="p-4 rounded-2xl bg-slate-50/70 border border-slate-200/80 space-y-1.5">
                            ...
                            <div class="pt-1">
                                <input
                                    ...
                                    name="limits[{{ $key }}]"
                                    id="limit_{{ $key }}"
                                    value="{{ old('limits.'.$key, $plan->getLimit($key, $limit['default'])) }}"
                                    required
                    ...
                    @foreach ($supportedFeatures as $fKey => $feature)
                        <label class="p-4 rounded-2xl border border-slate-200/80 bg-white hover:border-[#4b55c8]/40 hover:bg-[#f8faff] transition flex items-start gap-3 cursor-pointer group select-none">
                            <input
                                ...
                                name="features[]"
                                value="{{ $fKey }}"
                                {{ is_array($currentFeatures) && in_array($fKey, $currentFeatures) ? 'checked' : '' }}
            ...
            <div class="pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('super-admin.subscriptions.plans.show', $plan) }}" class="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-[#64748b] text-xs font-semibold transition">
                    Cancel
                </a>
                <button
                    type="submit"
