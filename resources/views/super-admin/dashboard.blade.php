...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-5 shadow-lg relative overflow-hidden group hover:border-slate-700 transition">
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Stores</span>
            ...
            <div class="mt-3 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
                ...
                <span class="font-mono text-teal-400">{{ $stats['total_stores'] }} Live</span>
            </div>
        </div>
        ...
        <div class="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-5 shadow-lg relative overflow-hidden group hover:border-slate-700 transition">
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Active Stores</span>
            ...
            <div class="mt-3 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
                <span>Status</span>
                <span class="text-emerald-400 font-medium">Active Tenants</span>
            </div>
        </div>
    ...
    <div class="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-5 sm:p-6 shadow-lg">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <!-- Add Store -->
            <button type="button" disabled class="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 text-left opacity-75 cursor-not-allowed hover:border-slate-700 transition flex items-center justify-between group">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xs font-bold text-white">Add Store</p>
                        <p class="text-[10px] text-slate-500">Register new tenant</p>
                    </div>
                </div>
                <span class="text-[9px] font-bold uppercase tracking-wider text-teal-400 bg-teal-500/10 px-1.5 py-0.5 rounded border border-teal-500/20">Soon</span>
            </button>
            ...
            <button type="button" disabled class="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 text-left opacity-75 cursor-not-allowed hover:border-slate-700 transition flex items-center justify-between group">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xs font-bold text-white">View Stores</p>
                        <p class="text-[10px] text-slate-500">Manage tenant accounts</p>
                    </div>
                </div>
                <span class="text-[9px] font-bold uppercase tracking-wider text-purple-400 bg-purple-500/10 px-1.5 py-0.5 rounded border border-purple-500/20">Soon</span>
            </button>
        ...
        <div class="lg:col-span-2 bg-slate-900/80 border border-slate-800/80 rounded-2xl p-5 sm:p-6 shadow-lg flex flex-col justify-between">
            <div>
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-xs text-slate-500 font-mono">{{ $stats['recent_stores']->count() }} records</span>
                </div>
                            ...
                            <tbody class="divide-y divide-slate-800/60">
                                @foreach ($stats['recent_stores'] as $store)
                                    <tr class="hover:bg-slate-800/30 transition">
                                        <td class="px-4 py-3 font-semibold text-white">{{ $store->name ?? 'Store #'.$store->id }}</td>
                                        <td class="px-4 py-3 text-slate-400">{{ $store->owner_name ?? '—' }}</td>
                                        <td class="px-4 py-3">
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <button disabled class="text-teal-400 hover:text-teal-300 font-semibold cursor-not-allowed">View</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                @else
                    ...
                    <div class="py-12 rounded-xl bg-slate-950/40 border border-slate-800/50 flex flex-col items-center justify-center text-center p-6">
                        ...
                        <p class="text-xs text-slate-500 max-w-sm mt-1">Medical stores will be listed here once the Store Management module is built in subsequent phases.</p>
                    </div>
                @endif
            </div>

            <div class="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-500">
                <span>Phase 2 Scope: Super Admin Dashboard</span>
                <span>Store Management coming in next phase</span>
            </div>
        </div>
